...
import { ThirdPersonMesh } from "../View/Mesh/ThirdPersonMesh";
...
export class InputManager implements IUpdatable {
    ...
    update(dt: number): void {
        ...
        if (this.keys.get(Key.One)?.justReleased) {
            const ak47 = Game.getInstance().globalLoadingManager.loadableMeshs.get("AK47")!.clone();
            playerRenderer?.setMesh(ak47 as FPSMesh);

        }
        if (this.keys.get(Key.Two)?.justReleased) {
            const usp = Game.getInstance().globalLoadingManager.loadableMeshs.get("Usp")!.clone();
            playerRenderer?.setMesh(usp as FPSMesh);
        }

        if (this.keys.get(Key.Three)?.justReleased) {
            const knife = Game.getInstance().globalLoadingManager.loadableMeshs.get("Knife")!.clone();
            playerRenderer?.setMesh(knife as FPSMesh);
        }

        if (this.keys.get(Key.Four)?.justReleased) {
            if (this.playerWrapper.switchToFpsView()) {
                this.playerWrapper.cameraManager = new FPSCameraManager(this.playerWrapper.player, this.playerWrapper.renderer!.camera);
        ...
        if (this.keys.get(Key.Five)?.justReleased) {
            if (this.playerWrapper.switchToTpsView()) {
                this.playerWrapper.cameraManager = new TPSCameraManager(this.playerWrapper.player, this.playerWrapper.renderer!.camera);
                Game.getInstance().renderer.setCamera(this.playerWrapper.cameraManager!.camera);
                this.playerWrapper.renderer?.setCameraManager(this.playerWrapper.cameraManager);
