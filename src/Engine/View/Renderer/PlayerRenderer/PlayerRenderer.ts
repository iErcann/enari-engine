...
export abstract class PlayerRenderer implements IUpdatable {
    ...
    constructor(player: Player) {
        ...
        if (this.showDebug) {
            ...
            const fov = debugUI.addInput(this, "baseFov").on("change", () => this.setFov(this.baseFov));
            ...
            const magn = debugUI.addMonitor(this.player, "currentSpeedMagnitude", { title: "Player current speed magnitude" });
    ...
    protected createDebugMeshs(): void {
        ...
        this.game.addToRenderer(this.debugCollisionLine);
        {
            ...
            const geometry: THREE.BoxGeometry = new THREE.BoxGeometry(this.player.capsuleDimension.x, this.player.capsuleDimension.y * 2, this.player.capsuleDimension.x);
    ...
    public handleShoot(hitscanResult: HitscanResult): void {
        ...
        if (this.showDebugHitscan) {
            ...
            if (hitscanResult.hasHit) {
                ...
                var dotMaterial = new THREE.PointsMaterial({ size: 0.1, color: 0xF029302 });
