...
export class Game implements IUpdatable {
    ...
    public setPhysicsObjects(): void {
        ...
        for (let j = 0; j < 5; j++) {
            const cube = new CubeRenderer(new Vector3D(0, 2, 5), new Vector3D(0, 0, 0), new Vector3D(4, 4, 4), 10);
        ...
        for (let j = 0; j < 5; j++) {
            const sphere = new SphereRenderer(new Vector3D(0, 2, 5 * j), new Vector3D(0, 0, 0), new Vector3D(j, j, j), 10);
