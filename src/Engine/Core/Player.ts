...
export class Player extends Pawn implements IUpdatable {
    ...
    constructor(position: Vector3D) {
        ...
        const shape = this.createShape(new Vector3D(this.capsuleDimension.x, this.capsuleDimension.y, this.capsuleDimension.x));
    ...
    private raycastToGround(): void {
        ...
        const from: Ammo.btVector3 = this.position.clone().add(new Vector3D(initialLocalPos.x, initialLocalPos.y, initialLocalPos.z)).toAmmo();
        const to: Ammo.btVector3 = this.position.clone().add(new Vector3D(initialLocalPos.x, initialLocalPos.y - size, initialLocalPos.z)).toAmmo();

        const rayCallBack = new Ammo.ClosestRayResultCallback(from, to);
        this.world.rayTest(from, to, rayCallBack);
        if (!this.isOnGround && rayCallBack.hasHit())
            this.velocityPreserveAcc = 0;
    ...
    private Accelerate(accelDir: Vector3D, prevVelocity: Vector3D, wishSpeed: number, airAccel: number, dt: number): Vector3D {
    ...
    private MoveGround(accelDir: Vector3D, prevVelocity: Vector3D, dt: number): Vector3D {
        const friction = 1;
        const speed = Math.pow(prevVelocity.x, 2)
            + Math.pow(prevVelocity.z, 2);
        if (speed != 0)  
        {
    ...
    update(dt: number): void {
        ...
        this.currentSpeedMagnitude = Math.pow(linearVelocity.x(), 2)
            + Math.pow(linearVelocity.z(), 2);
    ...
    public shoot(): HitscanResult {
        ...
        const rayCallBack = new Ammo.ClosestRayResultCallback(fromAmmo, toAmmo);
        ...
        if (hitScanResult.hasHit = rayCallBack.hasHit()) {
            ...
            const collisionBody: Ammo.btRigidBody = Ammo.btRigidBody.prototype.upcast(object);
    ...
    public jump(): void {
        console.log("jump")
        const vec3 = new Ammo.btVector3(0, this.jumpVelocity, 0);
