...
export class InputManager implements IUpdatable {
  ...
  constructor() {
    ...
    document.body.ownerDocument.addEventListener(
      "keydown",
      this.boundOnKeyDown,
      false
    );
  ...
  onMouseMove(evt: any): void {
    if (this.isLocked === false || !this.playerWrapper.player.isCurrentPlayer)
      return;
  ...
  update(dt: number): void {
    ...
    const speed =
      (this.keys.get(Key.Shift)?.isPressed
        ? 30
        : this.playerWrapper.player.speed) * dt;
    ...
    if (this.keys.get(Key.One)?.justReleased) {
      const ak47 = Game.getInstance()
        .globalLoadingManager.loadableMeshs.get("AK47")!
        .clone();
      playerRenderer?.setMesh(ak47 as FPSMesh);
    }
    if (this.keys.get(Key.Two)?.justReleased) {
      const usp = Game.getInstance()
        .globalLoadingManager.loadableMeshs.get("Usp")!
        .clone();
      playerRenderer?.setMesh(usp as FPSMesh);
    }

    if (this.keys.get(Key.Three)?.justReleased) {
      const knife = Game.getInstance()
        .globalLoadingManager.loadableMeshs.get("Knife")!
        .clone();
      playerRenderer?.setMesh(knife as FPSMesh);
    }
    ...
    if (this.keys.get(Key.Four)?.justReleased) {
      if (this.playerWrapper.switchToFpsView()) {
        ...
        Game.getInstance().renderer.setCamera(
          this.playerWrapper.cameraManager!.camera
        );
        this.playerWrapper.renderer?.setCameraManager(
          this.playerWrapper.cameraManager
        );
      }
    }
  ...
  onKeyDown(event: KeyboardEvent): void {
    ...
    if (event.key.toLowerCase() === "z" || event.key.toLowerCase() === "w")
      this.keys.get(Key.Forward)?.setPressed(true);
    if (event.key.toLowerCase() === "q" || event.key.toLowerCase() === "a")
      this.keys.get(Key.Left)?.setPressed(true);
    if (event.key.toLowerCase() === "s")
      this.keys.get(Key.Backward)?.setPressed(true);
    if (event.key.toLowerCase() === "d")
      this.keys.get(Key.Right)?.setPressed(true);
    if (event.key.toLowerCase() === "r")
      this.keys.get(Key.Reload)?.setPressed(true);

    if (event.key.toLowerCase() === "shift")
      this.keys.get(Key.Shift)?.setPressed(true);
    if (event.key.toLowerCase() === "e")
      this.keys.get(Key.Use)?.setPressed(true);
    if (event.key.toLowerCase() === "&" || event.key.toLowerCase() === "1")
      this.keys.get(Key.One)?.setPressed(true);
    if (event.key.toLowerCase() === "é" || event.key.toLowerCase() === "2")
      this.keys.get(Key.Two)?.setPressed(true);
    if (event.key.toLowerCase() === '"' || event.key.toLowerCase() === "3")
      this.keys.get(Key.Three)?.setPressed(true);
    if (event.key.toLowerCase() === "'" || event.key.toLowerCase() === "4")
      this.keys.get(Key.Four)?.setPressed(true);
    if (event.key.toLowerCase() === "(" || event.key.toLowerCase() === "5")
      this.keys.get(Key.Five)?.setPressed(true);
  }
  onKeyUp(event: KeyboardEvent): void {
    ...
    if (event.key.toLowerCase() === "&" || event.key.toLowerCase() === "1")
      this.keys.get(Key.One)?.onKeyUp();
    if (event.key.toLowerCase() === "é" || event.key.toLowerCase() === "2")
      this.keys.get(Key.Two)?.onKeyUp();
    if (event.key.toLowerCase() === '"' || event.key.toLowerCase() === "3")
      this.keys.get(Key.Three)?.onKeyUp();
    if (event.key.toLowerCase() === "'" || event.key.toLowerCase() === "4")
      this.keys.get(Key.Four)?.onKeyUp();
    if (event.key.toLowerCase() === "(" || event.key.toLowerCase() === "5")
      this.keys.get(Key.Five)?.onKeyUp();
  }
