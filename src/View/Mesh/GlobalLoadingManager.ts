...
export class GlobalLoadingManager extends THREE.LoadingManager {
  ...
  public loadableMeshs: Map<string, LoadableMesh> = new Map<
    string,
    LoadableMesh
  >();
  ...
  async loadAllMeshs() {
    ...
    const ak = new FPSMesh(
      "fps_mine_sketch_galil.glb",
      "AK47",
      new Vector3D(0, -0.03, 0.07)
    );
    await ak.load();
    ak.register(this.loadableMeshs);

    const usp = new FPSMesh(
      "fps_mine_sketch_compressed.glb",
      "Usp",
      new Vector3D(-0.09, 0.26, 0.35)
    );
    await usp.load();
