...
export class Player extends Pawn implements IUpdatable {
  ...
  constructor(position: Vector3D) {
    ...
    const shape = this.createShape(
      new Vector3D(
        this.capsuleDimension.x,
        this.capsuleDimension.y,
        this.capsuleDimension.x
      )
    );
  ...
  private raycastToGround(): void {
    ...
    const from: Ammo.btVector3 = this.position
      .clone()
      .add(
        new Vector3D(initialLocalPos.x, initialLocalPos.y, initialLocalPos.z)
      )
      .toAmmo();
  ...
  private MoveGround(
    ...
    if (speed != 0) {
      ...
      prevVelocity.multiplyScalar(
        (this.deceleration.x * Math.max(speed - drop, 0)) / speed
      );
    }
  ...
  update(dt: number): void {
    ...
    this.currentSpeedMagnitude =
      Math.pow(linearVelocity.x(), 2) + Math.pow(linearVelocity.z(), 2);

    if (
      colWithAnything &&
      this.velocityPreserveAcc > this.velocityPreserveDelay
    ) {
  ...
  public canShoot(): boolean {
    ...
    return (
      new Date().getTime() - this.lastShootTimeStamp.getTime() > this.rateOfFire
    );
  }
  public createHitscanPoints(): HitscanProperty {
    const from = this.position.clone().add(new Vector3D(0, this.eyeOffsetY, 0));
    const to = new Vector3D().addVectors(
      from,
      this.lookingDirection.clone().multiplyScalar(10000)
    );
    return {
      from,
      to,
  ...
  public shoot(): HitscanResult {
    ...
    const rayCallBack = new AmmoInstance!.ClosestRayResultCallback(
      fromAmmo,
      toAmmo
    );
    ...
    if ((hitScanResult.hasHit = rayCallBack.hasHit())) {
      ...
      const collisionBody: Ammo.btRigidBody =
        AmmoInstance!.btRigidBody.prototype.upcast(object);
  ...
  public canResetRecoil(): boolean {
    ...
    return (
      new Date().getTime() - this.lastShootTimeStamp.getTime() >
      this.rateOfFire * 2
    );
  }
