...
export class PlayerWrapper {
  ...
  public switchToFpsView(): boolean {
    ...
    const cameraManager = new FPSCameraManager(
      this.player,
      this.renderer!.camera
    );
    this.renderer!.setCameraManager(cameraManager);
    this.renderer!.onEnabled(
      this.tpsRenderer ? this.tpsRenderer.camera : undefined
    );
    return true;
  }
  ...
  public switchToTpsView(): boolean {
    ...
    const cameraManager = new TPSCameraManager(
      this.player,
      this.renderer!.camera
    );
    this.renderer!.setCameraManager(cameraManager);
    this.renderer!.onEnabled(
      this.fpsRenderer ? this.fpsRenderer.camera : undefined
    );
  ...
  static default(): PlayerWrapper {
    const player = new Player(new Vector3D(30, 10, 30));
